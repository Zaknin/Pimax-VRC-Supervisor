...
if (commandLineArgs.Any(arg => string.Equals(arg, "pimax-usb-port-cycle-experiment-json", StringComparison.OrdinalIgnoreCase)))
{
    var diagnosticConfig = SupervisorConfig.Load(configPath);
    var request = PimaxUsbPortCycleRequest.Parse(commandLineArgs);
    var result = await new PimaxUsbPortCycleExperimentRunner(new WindowsPimaxUsbPortCycleStateCollector(diagnosticConfig)).RunAsync(request, shutdown.Token);
    Console.WriteLine(JsonSerializer.Serialize(result, PimaxUsbPortCycleJson.Options));
    return;
}

if (commandLineArgs.Any(arg => string.Equals(arg, "pimax-usb-paired-port-cycle-design-json", StringComparison.OrdinalIgnoreCase)))
{
    var result = await PimaxUsbPairedPortCycleDesignCommand.RunAsync(shutdown.Token);
    Console.WriteLine(JsonSerializer.Serialize(result, PimaxUsbPairedPortCycleDesignJson.Options));
    return;
}

if (commandLineArgs.Any(arg => string.Equals(arg, "pimax-usb-paired-port-cycle-experiment-json", StringComparison.OrdinalIgnoreCase)))
{
    var diagnosticConfig = SupervisorConfig.Load(configPath);
    var request = PimaxUsbPairedExperimentRequest.Parse(commandLineArgs);
    var result = await new PimaxUsbPairedExperimentRunner(new WindowsPimaxUsbPortCycleStateCollector(diagnosticConfig)).RunAsync(request, shutdown.Token);
    Console.WriteLine(JsonSerializer.Serialize(result, PimaxUsbPairedExperimentJson.Options));
    return;
}
