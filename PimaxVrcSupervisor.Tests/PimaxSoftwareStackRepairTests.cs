...
public sealed class PimaxSoftwareStackRepairTests
{
    ...
    public async Task TargetCatalogTreatsPimaxClientAsCoupledGroupMember()
    {
        ...
        Assert.Contains(snapshot.Targets, item => item.TargetType == "processGroup" && item.Classification == PimaxRepairTargetClassification.RestartRecipeIncomplete && item.RestartRecipe?.Complete == false && item.LiveValidationRequired);
