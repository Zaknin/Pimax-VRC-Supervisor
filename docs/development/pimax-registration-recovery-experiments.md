...
The runtime-service restart experiment is a separate CLI-only experiment. It performs read-only discovery first, produces a dry-run confirmation token, and uses a one-shot UAC helper for the single privileged service stop/start. The parent CLI process may remain non-elevated. The helper is bound to the dry-run request, revalidates the exact service identity independently, writes an atomic JSON result, and may control only that one service.
...
| 3 | Restart relevant Pimax runtime/service only | Awaiting registration, exact service identity known. | Restart only the narrow runtime/service target through a UAC one-shot helper after dry-run token confirmation. | Service re-reads present power-on devices. | `RegisteredReady`. | Service restarts but state remains awaiting registration, or service cannot be safely restored. | Bounded stop/start and observation windows. | One safety-restoration start for the same service if normal start fails. | UAC helper only | SteamVR must be closed. | Medium-high | Service identity, dependency state, helper result, assessment timeline. | Implemented as `pimax-recovery-experiment-json --experiment restart-runtime-service`; hardware result pending |
