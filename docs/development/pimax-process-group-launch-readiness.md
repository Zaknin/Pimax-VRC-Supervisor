...
`pimax-shell-activation-precondition-json` is the read-only B2D-VG2 quiescence gate. It reports schema `pimax-shell-activation-precondition-v1` and keeps general software health separate from activation eligibility. The accepted state `quiescentForShellActivation` means launch-owned members are absent, permitted persistent members are stable for three one-second samples, stale registration evidence is non-blocking only because the group is intentionally stopped, no unknown Pimax-root process remains, and no duplicate installation or recovery lease is present.

The launch-owned members that block activation are `PimaxClient`, `DeviceSetting`, `PiPlayService`, `PiService`, `pi_server`, `PVRHome`, `pi_overlay`, `lighthouse_console`, `launcher`, and `fastlist-0.3.0-x64`. Permitted persistent members are `PiPlatformService_64`, `Tobii VR4PIMAXP3B Platform Runtime`, service-owned `PiServiceLauncher` when it is present, and `vrss_gaze_provider` only when it is the exact session-0 `C:\Program Files\Pimax\Runtime\vrss_gaze_provider.exe` instance with stable file hash evidence and approved `PiServiceLauncher` service-descendant provenance. `PiServiceLauncher` does not have to remain running after creating the VRSS child.

Phase 28D2-B2D-VG2 investigated the remaining VRSS instance and found the chain `services.exe -> PiServiceLauncher.exe -> vrss_gaze_provider.exe`; the launcher later exited while VRSS remained in session 0. The VRSS binary is currently unsigned, so the gate records `signatureState=unsigned` and never treats the VRSS file as trusted because of a signer. Trust comes from exact canonical path, session 0, single-instance stability, expected Pimax Runtime root, file hash/metadata recording, trusted signed `PiServiceLauncher` service configuration, and live or preserved provenance evidence. Parent-exited evidence is reported as reconstructed/probable unless the chain is observed directly during the current assessment.

`pimax-shell-activation-evidence-collect-elevated-json` and `pimax-shell-activation-evidence-assess-json` are the Phase 28D2-B2D-VG3 privilege-separated evidence handoff. Non-elevated process APIs can observe that session-0 VRSS exists, but they may not be able to read the live executable path or bind that process to disk identity. The elevated collector therefore requires an already-administrator token, refuses self-elevation, samples the session-0 process set three times, and writes one short-lived protected envelope under `C:\ProgramData\PimaxVrcSupervisor\ValidationEvidence`. It does not start or stop processes or services, request Shell activation, press Connect, or touch USB/DisplayPort.

The normal assessment command must run from the ordinary non-elevated Explorer desktop session. It verifies the fixed ProgramData path, owner, ACL, purpose, correlation ID, TTL, build/package binding, and envelope hash, then re-samples the current visible process set and binds the private session-0 VRSS identity to the still-running process. Successful assessment deletes the envelope and refuses success if deletion fails. The private envelope may contain raw process IDs and parent IDs; public JSON redacts those fields, usernames, machine names, command lines, and certificate serial numbers.

VG3 evidence can authorize only `pimaxShellActivationReadOnlyGateAssessment`. It cannot authorize live Shell activation. A later actual validation must collect a fresh envelope with a separate purpose and final live correlation ID.

Known Pimax VRC Supervisor executables are excluded from Pimax software-stack membership by exact Supervisor-owned identity, including `PimaxVrcSupervisor`, `PimaxVrcSupervisorWatcher`, `PimaxVrcSupervisorConfigurator`, `PimaxVrcSupervisorSteamVrHost`, and `PimaxVrcSupervisorTui`. This prevents the protected Phase 29D-E watcher from appearing in core, launch-owned, permitted-persistent, unclassified, or duplicate-installation Pimax member lists without hiding real Pimax runtime processes.
...
The B2D-V safe abort and VG2 read-only revalidation consume no activation budget: no correlation ID, no observer, no activation, `shellRequestCount=0`, and `retryCount=0`. VG2 may prove the corrected quiescent gate, but it still stops before the actual activation confirmation boundary.
