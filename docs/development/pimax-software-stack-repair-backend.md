...
VG3 adds privilege-separated read-only evidence handoff for the session-0 VRSS identity gap. `pimax-shell-activation-evidence-collect-elevated-json` must be launched from an already-administrator development shell; it refuses non-admin execution, never self-elevates, samples three times at one-second intervals, and atomically writes `pimax-shell-activation-evidence-v1` to `C:\ProgramData\PimaxVrcSupervisor\ValidationEvidence`. The protected directory disables inherited unsafe ACLs; SYSTEM and Administrators receive full control, while the active interactive user receives read and delete rights only on the individual evidence file. Standard users must not be able to create, replace, or modify envelopes or redirect the evidence path.

`pimax-shell-activation-evidence-assess-json` is the matching normal-session consumer. It refuses elevated, session-0, service, LocalSystem, noninteractive, or Explorer-mismatched contexts, verifies canonical ProgramData location, owner, ACL, correlation ID, purpose, TTL, package/build binding, boot/session token, and envelope content hash, then consumes and deletes the file once. Replay is refused for expired, copied, modified, wrong-purpose, wrong-correlation, wrong-build, post-reboot, process-restarted, second-VRSS, core-member-present, unknown-root, or duplicate-installation evidence.

The public assessment embeds the existing `pimax-shell-activation-precondition-v1` and `pimax-shell-activation-capability-v1` results so one-time evidence is not consumed twice. Private bindings may contain raw process IDs, parent process IDs, creation timestamps, and file identity; public JSON reports only sanitized identity evidence and continues to redact usernames, machine names, command lines, raw IDs, and certificate serial numbers. Unsigned VRSS remains acceptable only as `signatureState=unsigned` when exact path, session 0, hash, single-instance stability, and trusted signed launcher service configuration are proven. Direct live ancestry is `confirmed`; preserved exited-launcher provenance is only `probable`.

VG3 remains read-only. Evidence purpose is `pimaxShellActivationReadOnlyGateAssessment`, `backendExecutable=false`, `automaticRecoveryAllowed=false`, `tuiExposureAllowed=false`, `configuratorExposureAllowed=false`, and `watcherExecutionAllowed=false`. The actual Shell activation phase must collect a fresh envelope with a separate purpose and a separate final live correlation ID.
...
B2D also adds command-dispatch regression coverage for the B2C accidental first-run incident. Pimax development commands dispatch before interactive first-run, direct-launch migration, configuration migration, scheduled-task migration/rebinding, watcher startup, dashboard startup, and managed application startup. `pimax-shell-activation-evidence-collect-elevated-json`, `pimax-shell-activation-evidence-assess-json`, and `pimax-shell-activate-validation-json` are not exposed through the TUI, Configurator, watcher, or ordinary Supervisor action path. VG3 read-only revalidation runs only the elevated evidence collector and normal evidence assessment, then stops before final live correlation ID generation, live observer launch, or activation.
